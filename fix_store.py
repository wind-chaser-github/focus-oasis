import json

writes = []
with open('/Users/chaser/.gemini/antigravity-ide/brain/ac8d33e3-c641-4dfc-a916-af10ce57f4f7/.system_generated/logs/transcript.jsonl', 'r') as f:
    for line in f:
        try:
            data = json.loads(line)
            tool_calls = data.get('tool_calls', [])
            for call in tool_calls:
                if call.get('name') == 'write_to_file':
                    args = call.get('args', {})
                    if 'useTimerStore.ts' in args.get('TargetFile', ''):
                        c = args.get('CodeContent', '')
                        if c.startswith('"'):
                            try:
                                c = json.loads(c)
                            except:
                                pass
                        writes.append(c)
        except Exception as e:
            pass

for i, w in enumerate(writes):
    if 'export interface PlantRecord' in w and '<truncated' not in w:
        with open('src/store/useTimerStore.ts', 'w') as out:
            out.write(w)
        print("Restored FULL useTimerStore from write:", i)
        break
